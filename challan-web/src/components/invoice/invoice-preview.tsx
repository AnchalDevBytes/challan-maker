...
export default function InvoicePreview({ data, removeBranding }: PreviewProps) {
  ...
  // Scale the A4-style document to fit its container
  const containerRef = useRef<HTMLDivElement>(null);
  const docRef = useRef<HTMLDivElement>(null);
  const [scale, setScale] = useState(1);
  const DOC_WIDTH = 680; // natural render width of the doc
  ...
  const subTotal = currentDraft.items.reduce(
  ...
  return (
    ...
    <div ref={containerRef} className="w-full">
      ...
      <div
        style={{
          height: docRef.current ? docRef.current.offsetHeight * scale : "auto",
        }}
      >
        <div
          ref={docRef}
          ...
          className="bg-white shadow-lg border border-neutral-200 p-10 text-sm text-neutral-800 font-sans relative overflow-hidden rounded-xl"
        >
          {showBranding && (
            <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-[0.03]">
              <span className="text-9xl font-semibold -rotate-45 text-black text-center">
                CHALLAN MAKER
              </span>
            </div>
          )}
          ...
          <div className="relative z-10 space-y-6">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-xl font-bold text-neutral-900">
                  {currentDraft.senderDetails.name}
                </h1>
                <div className="text-neutral-500 whitespace-pre-line text-xs max-w-48">
                  {currentDraft.senderDetails.address}
                  {currentDraft.senderDetails.email && (
                    <div>{currentDraft.senderDetails.email}</div>
                  )}
                  {currentDraft.senderDetails.phone && (
                    <div>{currentDraft.senderDetails.phone}</div>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-between items-start">
              <div className="flex flex-col items-start">
                {(currentDraft.clientDetails.name ||
                ...
                <div className="text-neutral-500 whitespace-pre-line text-xs leading-relaxed">
                  {currentDraft.clientDetails.address}
                  {currentDraft.clientDetails.email && (
                    <div>{currentDraft.clientDetails.email}</div>
                  )}
                  {currentDraft.clientDetails.phone && (
                    <div>{currentDraft.clientDetails.phone}</div>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-1">
                  ...
                  {currentDraft.issueDate && (
                    <div className="flex justify-between gap-8">
                      ...
                      <span>
                        {format(
                          new Date(currentDraft.issueDate),
                          "dd MMM yyyy",
                        )}
                      </span>
                    </div>
                  ...
                  {currentDraft.dueDate && (
                    <div className="flex justify-between gap-8">
                      ...
                      <span>
                        {format(new Date(currentDraft.dueDate), "dd MMM yyyy")}
                      </span>
                    </div>
                  )}
                </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-2 border-neutral-900 text-xs uppercase tracking-wider">
                  <th className="py-3 font-bold text-neutral-900">
                    Description
                  </th>
                  <th className="py-3 text-right font-bold text-neutral-900">
                    Qty
                  </th>
                  <th className="py-3 text-right font-bold text-neutral-900">
                    Price
            ...
            <div className="grid grid-cols-2 gap-8 pt-8 border-t border-neutral-100">
              <div>
                {(currentDraft.bankDetails?.accountNumber ||
                  currentDraft.bankDetails?.bankName ||
                  currentDraft.bankDetails?.ifscCode ||
                  currentDraft.bankDetails?.accountName) && (
                  <h4 className="font-bold text-xs uppercase text-neutral-500 mb-1">
                    Bank Details
                  </h4>
                )}
              ...
              <div className="text-right">
                ...
                {currentDraft.terms && (
                  <>
                    <h4 className="font-bold text-xs uppercase text-neutral-500 mt-5">
                      Terms
                    </h4>
