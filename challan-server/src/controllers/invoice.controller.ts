...
export const uploadLogo = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(200).json(
            new ApiResponse(200, { url: result.url, publicId: result.publicId }, "Logo uploaded successfully")
        );
    } catch (error) {
        next(error);
    }
...
export const createInvoice = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if(result.deleteInvoiceId) {
            return { message: `Invoice created successfully. Oldest invoice (ID: ${result.deleteInvoiceId}) deleted to maintain limit of 5 invoices.`}
        }
...
export const updateInvoice = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const updateInvoice = await InvoiceService.updateInvoice(userId, invoiceId, bodyData);

        res.status(200).json(
            new ApiResponse(200, { invoice: updateInvoice }, "Invoice updated successfully")
        );
    } catch (error) {
        next(error);
    }
